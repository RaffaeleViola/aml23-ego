...
import torch.optim
import torch
from utils.loaders_mid import MidDataset
    ...
    if args.action == "train":
        ...
        for m in modalities:
            if args.resume_from is not None:
                action_classifier.load_last_model_an(args.resume_from, m)        # define number of iterations I'll do with the actual batch: we do not reason with epochs but with iterations
        # i.e. number of batches passed
        # notice, here it is multiplied by tot_batch/batch_size since gradient accumulation technique is adopted
        training_iterations = args.train.num_iter * (args.total_batch // args.batch_size)
        # all dataloaders are generated here
        train_loader = torch.utils.data.DataLoader(MidDataset(args.dataset.shift.split("-")[0], modalities,
                                                                       'train', args.dataset,
                                                                       args.train.num_frames_per_clip,
                                                                       args.train.num_clips, args.train.dense_sampling,
                                                                       args.train.spectogram, load_feat=True),
                                                   batch_size=args.batch_size, shuffle=True,
                                                   num_workers=args.dataset.workers, pin_memory=True, drop_last=True)

        val_loader = torch.utils.data.DataLoader(MidDataset(args.dataset.shift.split("-")[-1], modalities,
                                                                     'val', args.dataset,
                                                                     args.train.num_frames_per_clip,
                                                                     args.train.num_clips, args.train.dense_sampling,
                                                                     args.train.spectogram, load_feat=True),
